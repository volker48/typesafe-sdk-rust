...
async fn concurrent_calls_keep_retry_options_and_headers_independent() {
    ...
    let (retried, once) = tokio::join!(
        client.system_one_with(&request, &retry),
    ...
    for enabled in [true, false] {
        ...
        let task = tokio::spawn(async move {
            ...
            if enabled {
                ...
                second
                    .write_all(response(200, "", r#"{"model":"m","usage":{}}"#).as_bytes())
                    .await
                    .unwrap();
            } else {
...
async fn cancelling_a_pending_retry_leaves_no_background_request() {
    ...
    let call = tokio::spawn(async move { client.system_one(&request()).await });
